...
static int FT_findNode(const char *pcPath, Node_T *poNResult) {
   ...
   /* do not continue if FT is not initialized */
   if(!bIsInitialized) {
      *poNResult = NULL;
      return INITIALIZATION_ERROR;
   }

   /* create a path for the node */
...
boolean FT_containsFile(const char *pcPath) {
   ...
   iStatus = FT_findNode(pcPath, &oNFound);
   return (boolean) (iStatus == SUCCESS && 
      Node_isDirectory(oNFound) == FALSE);
}
...
int FT_rmFile(const char *pcPath) {
   ...
   /* if node is not in FT */
   if(iStatus != SUCCESS)
       return iStatus;

   if(Node_isDirectory(oNFound) == TRUE) 
       return NOT_A_FILE;
...
void *FT_replaceFileContents(const char *pcPath, void *pvNewContents,
   size_t ulNewLength) {
      ...
      if(oNFound != NULL) {
         if(Node_isDirectory(oNFound) == TRUE) return NULL;
         pvOldContents = Node_putContents(oNFound, pvNewContents, 
                                          ulNewLength);
...
int FT_stat(const char *pcPath, boolean *pbIsFile, size_t *pulSize) {
   ...
   if(Node_isDirectory(oNFound) == TRUE) {
      ...
      *pbIsFile = TRUE;
      *pulSize = Node_getContentsLength(oNFound);
   }
   return SUCCESS;
}
